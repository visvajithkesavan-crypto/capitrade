...
app.post('/api/bot/chat', async (c) => {
  ...
  try {
    ...
    const systemPrompt = [
      ...
      ``,
      `FORMAT RULES — follow these strictly every single response:`,
      `- NEVER write paragraphs. Every response must use bullet points.`,
      `- Structure every response as:`,
      `    • One sentence of observation or context (no bullet, just plain)`,
      `    • 2-3 bullet points maximum with your analysis`,
      `    • One question on its own line at the end`,
      `- Each bullet point must be ONE sentence only. No long bullets.`,
      `- Bold **one key phrase** per response using markdown bold.`,
      `- Total response length: 4-6 lines maximum. Never more.`,
      `- If you catch yourself writing a sentence longer than 20 words, split it into two bullets instead.`,
      ``,
      `TRADE CONTEXT:`,
      `- Phase: ${phase}`,
