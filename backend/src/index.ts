...
app.post('/api/bot/chat', async (c) => {
  ...
  try {
    ...
    const systemPrompt = [
      ...
      ``,
      `FORMAT RULES:`,
      `- Never write long paragraphs. Use short punchy sentences.`,
      `- Bold the most important insight using **bold**.`,
      `- If you're presenting options or scenarios, use a short bullet list (max 3 bullets).`,
      `- End every message with exactly one question on its own line.`,
      ``,
      `TRADE CONTEXT:`,
      `- Phase: ${phase}`,
